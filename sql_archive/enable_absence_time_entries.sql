...
-- 2. Neue Spalten für Absences hinzufügen
ALTER TABLE public.time_entries 
    ADD COLUMN IF NOT EXISTS absence_id UUID REFERENCES public.absences(id) ON DELETE CASCADE,
    ADD COLUMN IF NOT EXISTS entry_date DATE; -- Wichtig, da eine Absence mehrere Tage hat

-- 3. Bestehenden Unique Constraint entfernen (war shift_id, user_id)
...
-- Fall A: Einträge für Shifts (Darf nur 1 pro Shift & User geben)
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_shift_entry 
    ON public.time_entries(user_id, shift_id) 
    WHERE shift_id IS NOT NULL;

-- Fall B: Einträge für Absences (Darf nur 1 pro Absence-ID & Datum & User geben)
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_absence_entry 
    ON public.time_entries(user_id, absence_id, entry_date) 
    WHERE absence_id IS NOT NULL;

-- 5. Policies aktualisieren (falls nötig)
-- Unsere Policies basieren auf user_id, das passt also weiterhin.
-- Aber wir hatten eine komplexe CHECK Policy "is_month_locked", die via shifts ging.
-- Die müssen wir anpassen, dass sie auch bei Absences funktioniert.
