-- RLS Cleanup (Vereinfachte Version)
...
-- === SIGNATURES ===
CREATE POLICY "policy_signatures_user_own" ON public.signatures FOR ALL TO authenticated USING ( user_id = (select auth.uid()) );
CREATE POLICY "policy_signatures_admin_all" ON public.signatures FOR ALL USING ( exists (select 1 from public.profiles where id = (select auth.uid()) and role = 'admin') );
